...
-- CUSTOM STUFF
function sync_yank_to_osc52()
  -- Get contents of register 0 (the last explicit yank)
  local lines = vim.fn.getreg("0", 1, true)

  -- If register 0 is empty, fallback to the unnamed register
  if #lines == 1 and lines[1] == "" then
    lines = vim.fn.getreg('"', 1, true)
  end

  -- Send it directly via OSC 52 if we have content
  if #lines > 0 and not (#lines == 1 and lines[1] == "") then
    require("vim.ui.clipboard.osc52").copy("+")(lines)
    print("Yank synced to system clipboard via OSC 52!")
  else
...
nmap_leader('.', '<Cmd>lua sync_yank_to_osc52()<CR>', 'Reg to Ext Clipboard')
